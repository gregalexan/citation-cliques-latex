...
Within each subject, directed edge swaps preserve node in- and out-degree, and the observed cumulative weights are permuted across the rewired dyads.  We use 500 graph-null replicates and 500 independent within-pair label swaps.  Under the add-one empirical p-value rule, 500 draws give a minimum attainable value of $1/501$; this is a declared precision and runtime choice rather than a theoretical requirement.  The graph null is conditional on the observed maximal-clique membership sets: each rewired graph rescores those candidate groups with the same density and reciprocity rules.  This supplies an empirical comparison for the observed groups without allowing a changing number of alternative cliques to dominate the null.  Independent within-pair Case--Control label swaps supply a second null for Case membership share.  The anomaly flag is only a descriptive overlay and is not used to define the clique population.  In the canonical results, the reciprocal-clique count was unusual under the graph null ($p=0.0020$), while mean density ($p=0.4685$) and mean reciprocity ($p=0.1220$) were not individually unusual; the Case membership share was unusual under label swaps ($p=0.0020$).  This provides the requested graph definition, quantitative thresholds, directed adjustments, and null-model comparisons without treating a structural clique as evidence of intent.
...
For the author analysis, the Methods formally defines the loopless directed weighted graph $G_s$ and its binary undirected projection $U_s$.  It identifies the population, direction, weight, denominator, and missingness rule for every measure.  Direction is discarded only for $U_s$ clustering and clique membership; reciprocity, balance, tier mixing, and the clique density measure retain direction and fractional weights.  The label-swap $p$-value tests the within-tier fractional-weight share, while weighted assortativity is reported separately as a descriptive statistic.  The new clique tables and Figure~\ref{ms-fig:clique-null} provide the analytical and visual basis for the network characterization.

\location{Data and Cohort, Network Construction, Anomaly Screen, and network results; Figure~\ref{ms-fig:clique-null}; Tables~\ref{ms-tab:metric-definitions}, \ref{ms-tab:tier-mixing}, \ref{ms-tab:clique-summary}, and \ref{ms-tab:clique-sensitivity}}{83--104, 145--214, 261--279, and 314--345}
