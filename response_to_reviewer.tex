...
Within each subject, directed edge swaps preserve node in- and out-degree, and the observed cumulative weights are permuted across the rewired dyads.  The graph null is conditional on the observed maximal-clique membership sets: each rewired graph rescores those candidate groups with the same density and reciprocity rules.  This supplies an empirical comparison for the observed groups without allowing a changing number of alternative cliques to dominate the null.  Independent within-pair Case--Control label swaps supply a second null for Case membership share.  The anomaly flag is only a descriptive overlay and is not used to define the clique population.  This provides the requested graph definition, quantitative thresholds, directed adjustments, and null-model comparisons without treating a structural clique as evidence of intent.
