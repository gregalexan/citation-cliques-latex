...
- `tables/`, including paired estimates, balance, detector sensitivity,
  weighted mixing, and outlier components;
...
3. Match Case and Control observations without replacement within subject and
   on nearby subject-specific five-year h-index (`h5`).
...
seeds.  The resulting column `final_flag` is the only flag used by tables,
figures, component extraction, and enrichment calculations.  Reported Case
share and Case-versus-Control enrichment describe association, not classifier
precision and not evidence of manipulation.
