...
For a graph-based null, each subject graph is rewired with directed edge swaps that preserve every node's in-degree and out-degree.  The observed cumulative weights are then permuted across the rewired dyads within subject.  The null is conditional on the observed maximal-clique membership sets: after each rewiring, we rescore those candidate groups with the same density and reciprocity rules rather than allowing a changing number of alternative cliques to dominate the comparison.  We use 499 seeded null replicates.  The empirical upper-tail probability is $(1+n_{\geq})/(1+N)$, where $n_{\geq}$ is the number of conditional-null counts at least as large as observed and $N$ is the number of valid replicates.  A second null independently swaps Case and Control labels within matched pairs to test the Case share of clique memberships.  These tests establish whether the defined structural pattern is unusual under the stated nulls; they do not assign a motive to any group.
