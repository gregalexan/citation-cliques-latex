...
\providecommand{\CliqueFindingText}{Formal clique estimates are pending the canonical full-data rebuild.}
...
\title{Citation-Network Cohesion and Reciprocal Citation Cliques Across Journal-Impact Strata:\\
...
\begin{enumerate}
  ...
  \item How do fractional citation weights mix across the two tiers?
\end{enumerate}
...
\begin{description}
  ...
  \item[Matched-author induced graph.] For each subject $s$, this graph retains only nodes in the matched cohort for $s$ and the edges among them.  It defines reciprocity, local clustering, within-sample citation balance, and tier mixing.
...
For the network analysis, authors are nodes and citations are directed weighted edges.  Some measures follow each author's outgoing citations to any identified recipient; others use only edges between matched authors.  Direction is ignored only for the binary neighbor graph used for local clustering.
...
We test citation-clique structure on the full matched-author graph rather than on the anomaly flags.  A clique $C$ is a maximal clique in $U_s$ with at least four matched authors.  This definition requires every unordered member pair to have a positive citation in at least one direction; the direction and weight of those links are then evaluated separately.  For a clique of size $k$, directed density is
\begin{equation}
  d_C=\frac{m_C}{k(k-1)},
\end{equation}
...
\end{equation}
The primary rule requires $d_C\geq0.75$ and $r_C\geq0.50$.  We repeat it for minimum clique sizes $k=3,4,5$ and reciprocity thresholds $0.25,0.50,0.75$; the density threshold remains fixed at 0.75.  The canonical anomaly flag is counted as an overlay on these groups and does not define the clique population.

For a graph-based null, each subject graph is rewired with directed edge swaps that preserve every node's in-degree and out-degree.  The observed cumulative weights are then permuted across the rewired dyads within subject.  The null is conditional on the observed maximal-clique membership sets: after each rewiring, we rescore those candidate groups with the same density and reciprocity rules rather than allowing a changing number of alternative cliques to dominate the comparison.  We use 500 seeded graph-null replicates and 500 independent within-pair label swaps.  This is a declared precision and runtime choice, not a theoretical requirement.  With the add-one empirical upper-tail rule $(1+n_{\geq})/(1+N)$, 500 draws give a minimum attainable value of $1/501$.  A second null independently swaps Case and Control labels within matched pairs to test the Case share of clique memberships.  These tests establish whether the defined structural pattern is unusual under the stated nulls; they do not assign a motive to any group.
...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[
    ...
    \node[branch,below right=of edges] (screen) {Control-referenced screen\\enrichment and tier mixing};
...
Isolation Forest repeatedly partitions the Control profiles; profiles separated from typical Controls in fewer partitions receive higher departure scores \parencite{Liu2008}.  A row passes the detector criterion when its score exceeds the empirical 99th percentile among Controls in its subject.  This percentile is an explicit tail operating point, not a normality assumption or a uniquely optimal threshold.  A row also passes the cohesion restriction when at least two of prior-coauthor citation rate, reciprocity, clustering, and outgoing HHI exceed their respective Control percentiles.  Because these four measures also enter the detector, the intersection is an overlapping guardrail rather than independent corroboration.  The final flag is the intersection and is used for enrichment and figures.
...
\inputgeneratedtable{clique_summary}{tab:clique-summary}{Primary formal reciprocal-clique rule and comparison with degree-preserving rewired graphs.}

\inputgeneratedtable{clique_sensitivity}{tab:clique-sensitivity}{Sensitivity to minimum clique size and the weighted-reciprocity threshold.}

The analysis treats a clique as a structural configuration in the defined citation graph.  The conditional rewiring comparison tests whether the observed candidate groups retain their density and reciprocity more often than expected under preserved degree sequences, while the pair-label comparison tests whether clique memberships are concentrated in Cases.  Neither comparison changes the graph definition or turns a structural association into a claim about motive.
...
The regenerated evidence is summarized as follows.  \PrimaryFindingText\ \AnomalyFindingText\ \MixingFindingText\ \CliqueFindingText\ The detailed estimates, intervals, and sensitivity results in Section~\ref{sec:results} govern interpretation when a short summary masks heterogeneous directions or uncertainty.
...
The Control-referenced procedure should consequently be understood as triage.  It identifies profiles that are unusual under a declared reference distribution and adds a transparent cohesion restriction.  The two criteria deliberately share inputs, so their intersection is a guardrail rather than two independent lines of evidence.  It does not estimate the probability of manipulation, determine whether a citation was intellectually warranted, or establish intent.  Any research-integrity use would require independent validation from full texts, editorial records, author disambiguation, and domain experts.
