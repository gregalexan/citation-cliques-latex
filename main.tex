...
\begin{abstract}
We ask whether authors in lower-impact journal groups have different citation patterns from matched authors in higher-impact groups.  We call the lower-impact observations Cases and the higher-impact observations Controls.  We compare \MatchedPairCount\ author--subject pairs within subject and at similar recent citation productivity.  \PrimaryFindingText\ \CliqueFindingText\ Lower-impact-group authors were more likely than matched higher-impact-group authors to belong to formally defined reciprocal citation cliques.
\end{abstract}
...
\begin{enumerate}
  \item Do matched Case and Control authors differ in citation-network cohesion, outgoing citation concentration, and reciprocal-clique membership?
...
The primary cohort contains \MatchedPairCount\ matched Case--Control pairs across the five subjects.  It is built from every non-empty ORCID in the fixed snapshot that meets the portfolio, work-count, and positive-$h_5$ eligibility rules; no final-character sampling rule is applied.

\section{Matching}
\label{sec:matching}

Recent subject-specific citation productivity is summarized by $h_5$, the Hirsch index calculated from an author's works in subject $s$ published during 2020--2024 \parencite{Hirsch2005}.  In ordinary language, $h_5$ is the largest number of works that each have at least that many citations in the fixed snapshot.  The matching pool requires $h_5\geq1$ and excludes otherwise group-eligible zero-$h_5$ profiles before candidate generation.
...
An independent audit repeats the deterministic match from the generated full-cohort profiles and compares it with the materialized pairs.  It requires identical order and membership, no pair beyond the three-point limit, and no reused author--subject key.  The complete audit accompanies the replication materials.
...
\end{equation}
The primary rule requires at least 75\% of all possible directed links and a weighted reciprocity of at least 0.50.  We repeat it for minimum clique sizes $k=3,4,5$ and reciprocity thresholds $0.25,0.50,0.75$; the density threshold remains fixed at 0.75.

For inference, overlapping qualifying cliques are converted to one binary membership value per author--subject.  Case and Control membership rates are compared within the original matched pairs.  The exact two-sided null distribution conditions on the number of discordant pairs and assigns either member of each discordant pair equal probability of carrying the clique-membership label.  We report both discordant-pair counts, the paired rate difference, the exact $p$-value, and the direction of the rate difference within each subject.
...
\begin{figure}[p]
  ...
  \begin{tikzpicture}[
    ...
    branch/.style={draw,rounded corners,align=center,text width=0.22\textwidth,minimum height=13mm,fill=blue!5},
    ...
    \node[branch,below=of edges,xshift=-0.31\textwidth] (paired) {Paired inference\\four primary and four secondary outcomes};
    \node[branch,below=of edges] (cliques) {Reciprocal-clique analysis\\unique matched membership};
    \node[branch,below=of edges,xshift=0.31\textwidth] (screen) {Control-referenced screen\\enrichment and group mixing};
    \draw[arrow] (data) -- (venues);
    \draw[arrow] (venues) -- (cohort);
    \draw[arrow] (cohort) |- (edges);
    \draw[arrow] (data) |- (edges);
    \draw[arrow] (edges) -- (paired);
    \draw[arrow] (edges) -- (cliques);
    \draw[arrow] (edges) -- (screen);
  \end{tikzpicture}
  \caption{Reader-facing study design.  Venue classification and author matching define the cohort; subject-keyed citation construction then supports the paired, clique, and screening analyses.}
...
An author--subject enters the screen only when it has at least one identified non-self outgoing citation.  The screen uses five measures of an author's citation links: self-reference, citations to previous collaborators, mutual exchange, connections among neighbors, and concentration among recipients.  These inputs capture author-edge cohesion or concentration.  Same-journal referencing is excluded because it is defined on work references, citation balance because it requires internal incident weight, and year-to-year increase because it is temporal; all three are reported separately.  All five screen inputs must be observed, and an unavailable value is not changed to zero.

Within each subject, every screen input is centered at the Control median and divided by the Control interquartile range (IQR).  A nonfinite or zero Control IQR is replaced by 1 only to avoid division by zero.  Controls therefore define the preprocessing, detector fit, and thresholds; Cases do not affect those reference quantities.

No additional feature weights are applied, and the empirical percentile threshold makes no Gaussian assumption.
...
\inputgeneratedtable{clique_summary}{tab:clique-summary}{Matched reciprocal-clique membership comparison.}

\IfFileExists{\GeneratedRoot/figures/clique_membership.pdf}{%
  \begin{figure}[H]
    \centering
    \includegraphics[width=0.72\linewidth]{\GeneratedRoot/figures/clique_membership.pdf}
    \caption{Unique author--subject membership rates in primary-rule reciprocal citation cliques.  Authors in overlapping cliques are counted once.}
    \label{fig:clique-membership}
  \end{figure}
}{ }

\inputgeneratedtable{clique_sensitivity}{tab:clique-sensitivity}{Sensitivity of the clique rule.}

Here, a clique is a group of at least four matched authors in which every pair is connected by a citation in at least one direction.  The primary inference compares the binary author--subject membership indicator within pairs rather than counting repeated appearances in overlapping cliques.  There were \CliqueCaseOnlyPairs\ pairs in which only the Case belonged and \CliqueControlOnlyPairs\ in which only the Control belonged; the exact matched result was $p\CliqueExactP$.  The subject-level breakdown is reported with every threshold sensitivity.
...
\PrimaryDetailedFindingText\ This comparison complements the author-level concentration measures of Evdaimon et al., while measuring the distribution of references sent by an author rather than the number of authors needed to account for received citations \parencite{Evdaimon2024}.

The secondary results add detail.  \SecondaryDetailedFindingText\ These measures describe different parts of citation behavior from the journal-level citation stacking studied by Heneberg \parencite{heneberg2016}.  \AnomalyFindingText\ \MixingFindingText\ These are descriptive results that show how the groups differ; they do not identify a cause.

The clique analysis makes the main pattern more specific.  \CliqueFindingText\ Membership is defined once per author--subject even when cliques overlap, and the exact test preserves the matched design.  The evidence therefore concerns which matched authors belong to the defined structure, while leaving the reasons for the pattern open.

Kojaku et al.\ detect anomalous groups of journals by comparing journal-level citation structure with network expectations \parencite{Kojaku2021}.  Our result instead compares author-level clique membership within matched pairs under a declared structural rule.  The approaches share a relational perspective, but their populations, estimands, and null comparisons differ, so their estimates are not numerically interchangeable and neither structural result alone establishes misconduct.
...
The five broad subject categories cannot capture fine topical proximity.  The journal classifier was not independently validated against blinded human coding, and the publication-weighted Eigenfactor cutoffs remain sensitive to the observed journal network.  ORCID availability, the 70\% portfolio rule, and the positive-$h_5$ matching pool define the study population.  Matching on subject-specific $h_5$ improves comparability within that pool but does not balance geography, language, institution, team structure, career age, or unmeasured research topic.
...
Lower-impact-group authors were more likely than matched higher-impact-group authors to belong to formally defined reciprocal citation cliques.  The paired membership difference was \CliquePairedDifferencePoints\ percentage points (exact $p\CliqueExactP$).  This is an associational result for the defined cohort and supplies a pattern for follow-up without assigning a cause or intent.
...
The v4.0.0 software and aggregate replication materials for this revision are archived on Zenodo \parencite{dataset_zenodo}.  The fixed raw snapshot is not redistributed.
...
Cases and Controls require a non-empty ORCID, at least three eligible works, and a 70\% portfolio share in the relevant journal group.  The matching pool additionally requires a positive $h_5$ value; zero-$h_5$ profiles are not candidates.  Within that pool, candidate eligibility uses the inclusive hard caliper $|h_{5,\mathrm{Case}}-h_{5,\mathrm{Control}}|\leq3$.  Greedy selection proceeds without replacement within subject in the total order subject, ascending absolute $h_5$ distance, Case ORCID, and Control ORCID.  The generated cohort must pass the same subject-keyed positive-$h_5$ join and caliper checks.  Exact-$h_5$ pairs form a sensitivity subset rather than a replacement cohort.
...
The anomaly score is the negative of the library's \texttt{score\_samples} output.  The library's \texttt{contamination=auto} decision cutoff is not used; the reported cutoff is computed from the subject-specific Control scores.
...
\begin{itemize}
  ...
  \item generated pairs have complete subject-keyed $h_5$ records, satisfy the inclusive three-point caliper, and do not reuse an author--subject key;
...
SQL fixtures separately test fractional weight, null endpoints, work-reference deduplication, subject retention, and prior-coauthor timing.  Python unit tests cover full-ORCID cohort construction, matching eligibility and deterministic order, dynamic cohort validation, metric formulas, paired inference, unique clique membership, exact matched clique inference, detector overlap, feature ablation, weighted mixing, and flag reuse.
